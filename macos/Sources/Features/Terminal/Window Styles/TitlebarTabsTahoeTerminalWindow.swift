...
class TitlebarTabsTahoeTerminalWindow: TransparentTitlebarTerminalWindow, NSToolbarDelegate {
    ...
    private func isEmptyTitlebarSpace(at windowPoint: NSPoint) -> Bool {
        // Check each interactive view using hitTest — more robust than manual
        // bounds checking. Note: hitTest requires the point in the view's
        // superview's coordinate space.

        // Toggle buttons — any hit inside them means not empty
        if hitTestView(fileBrowserButton, at: windowPoint) != nil { return false }
        if hitTestView(markdownButton, at: windowPoint) != nil { return false }

        // Standard window buttons (traffic lights)
        for buttonType: NSWindow.ButtonType in [.closeButton, .miniaturizeButton, .zoomButton] {
            guard let button = standardWindowButton(buttonType) else { continue }
            if hitTestView(button, at: windowPoint) != nil { return false }
        }

        // Tab bar: interactive only if we hit a child element (button, tab, etc.)
        // The hosting view itself represents empty space within the tab bar.
        if let hitView = hitTestView(customTabBarHostingView, at: windowPoint),
           hitView !== customTabBarHostingView {
            return false
        }
    ...
    /// Hit test a view using the correct coordinate space (superview's coords).
    private func hitTestView(_ view: NSView, at windowPoint: NSPoint) -> NSView? {
        guard let superview = view.superview else { return nil }
        return view.hitTest(superview.convert(windowPoint, from: nil))
    }
